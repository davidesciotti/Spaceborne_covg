...
#         ell_tab = np.genfromtxt(cfg['ell_tab_path'])
#         ell_values = ell_tab[0]
#         delta_values = ell_tab[1]
#         ell_lo_edges = ell_tab[2]
#         ell_hi_edges = ell_tab[3]
#         nbl = len(ell_values)
...
ells_unbinned = np.arange(cfg['ell_max'])
...
ell_mask, cl_mask, fsky = get_mask_cl(mask)
...
if cfg['apodize_mask'] and int(survey_area_deg2) != int(utils.DEG2_IN_SPHERE):
    mask = mask.astype('float64', copy=False)
...
if cfg['nmt_ell_binning'] == 'linear':
    ...
    bin_obj = nmt.NmtBin.from_edges(
        ell_ini=ell_lo_edges.astype(int), ell_end=ell_hi_edges.astype(int)
    )
...
print(
    f'Coupling coefficients computed in '
...
nbl_4covsb = len(ells_4covsb)
delta_ells_4covsb = np.ones(nbl_4covsb)  # since it's unbinned
cl_GG_4covsb = deepcopy(cl_GG_unbinned[:, :zbins_use, :zbins_use])
cl_GL_4covsb = deepcopy(cl_GL_unbinned[:, :zbins_use, :zbins_use])
cl_LL_4covsb = deepcopy(cl_LL_unbinned[:, :zbins_use, :zbins_use])

np.save(f'{output_folder}/psky_merge_check/cl_tt_unb', cl_GG_unbinned)
np.save(f'{output_folder}/psky_merge_check/cl_te_unb', cl_GL_unbinned)
np.save(f'{output_folder}/psky_merge_check/cl_ee_unb', cl_LL_unbinned)
...
noise_3x2pt_5d = np.repeat(
    noise_3x2pt_4d[:, :, np.newaxis, :, :], nbl_4covsb, axis=2
)
...
# save all the ingredients that enter the functions below in an npz archive
np.savez(
    f'{output_folder}/psky_merge_check/ingredients_old',
    cl_tt=cl_tt_4covnmt,
    cl_te=cl_te_4covnmt,
    cl_ee=cl_ee_4covnmt,
    cl_tb=cl_tb_4covnmt,
...
if cfg['load_sample_cov']:
    ...
    cov_sim_10d = np.ones_like(cov_nmt_10d)
