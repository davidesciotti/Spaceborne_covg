...
def produce_correlated_maps(cl_TT, cl_EE, cl_BB, cl_TE, cl_EB, cl_TB, nreal, nside, zbins_use):
    ...
    for _ in tqdm(range(nreal)):
        ...
        corr_maps_ll = [hp.alm2map_spin([Elm, Blm], nside, 2, lmax) for (Elm, Blm) in corr_Elms_Blms]
...
if not part_sky:
    ...
    hp_pcl_tot = hp.anafast([_map_t * mask, _map_q * mask, _map_u * mask], lmax=lmax_eff)
    ...
    corr_maps_ll = [hp.alm2map_spin([Elm, Blm], nside, spin=2, lmax=lmax_eff) for (Elm, Blm) in corr_Elms_Blms]
    ...
    ax[1].axvline(x=elem_autpluscross_use, color='black', linestyle='--', alpha=.7)

    x_coords = [elem_auto_use / 2, (elem_autpluscross_use - elem_auto_use) / 2 +
                elem_auto_use, elem_auto_use / 2 + elem_autpluscross_use]
    labels = ['LL', 'GL', 'GG']
